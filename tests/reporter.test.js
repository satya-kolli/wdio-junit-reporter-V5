...
describe('wdio-junit-reporter', () => {
    ...
    it('should prepare name', () => {
        expect(reporter.prepareName()).toBe('Skipped test')
        expect(reporter.prepareName('Chrome something_odd.foobar')).toBe('Chrome something odd foobar')
    })
    ...
    it('has a suiteNameFormat option for setting names manually', () => {
        ...
        expect(reporter.prepareName('Skipped_foobar_test')).toBe('Skipped_ _test')
    })

    it('can format output', () => {
        expect(reporter.format()).toBe(undefined)
        expect(reporter.format(1)).toBe('1')
        expect(reporter.format({ a: 'foo', b: 'bar' })).toBe('{"a":"foo","b":"bar"}')
        expect(reporter.format({ a: 'foobar'.repeat(100) })).toBe('{"a":"[base64] 600 bytes"}')
        expect(reporter.format({ a: '@'.repeat(10000) })).toBe(`{"a":"${'@'.repeat(200)} ... (9800 more bytes)"}`)
        expect(reporter.format({ a: '@'.repeat(100).split('') })).toBe('{"a":["@","@","@","@","@","@","@","@","@","@","(90 more items)"]}')
